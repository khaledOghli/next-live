...
render(<App prop="x"/>)                           // explicit render call`}
  caption="export default is the unambiguous form. The others are recovered heuristically for react-live compatibility."
/>

Prefer `export default` in anything you store; the heuristics exist for
migration, not as a style to adopt.
