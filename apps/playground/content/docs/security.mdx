...
<CodeBlock title="proxy.ts" code={`const RUNNER_ROUTES = ['/apps', '/playground', '/docs'];

const needsEval =
  isDev || RUNNER_ROUTES.some((r) => pathname.startsWith(r));
...
Working copy: `apps/playground/proxy.ts`. Visit `/rsc-check` — not a runner route — to see CSP blocking eval.
