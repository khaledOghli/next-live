...
<Callout variant="warning" title="Sandbox mode cannot do this">
  Sharing works because snippets run in your page. In [sandbox mode](/docs/sandbox) they run in an isolated iframe instead, so props are copied as plain data: no live store, no functions by reference. Share by reference with authors you trust, and use the sandbox for everyone else.
</Callout>
