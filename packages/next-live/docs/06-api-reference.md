...
Builds an explicit module record, for the one shape that cannot be inferred: an
object wanting a named export literally called `default`.
...
| Export | Purpose |
|---|---|
| `compile(input)` | Transpile + resolve + evaluate. Returns `{ renderable, via, code }`. |
...
`RenderBudgetOptions`, `GlobResult`, plus the props type for each component.
